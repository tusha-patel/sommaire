    ...
    return <div className='flex items-start gap-2 sm:gap-4  '>
        ...
        <div className="flex-1 min-w-0">
            <h3 className='text-base xl:text-lg font-semibold text-gray-900 truncate w-4/5 '>
                {title || formatDistanceToNow(fileUrl)}
            </h3>
...
const StatusBadge = ({ status }: { status: string }) => {
    return <span className={cn('px-3 py-1 text-xs rounded-full capitalize', status === 'completed' ? "bg-green-100 text-green-800" : "bg-yellow-100 text-yellow-800 ")}>
        {status}
    </span>
}
...
const SummaryCard = ({ summary }: { summary: any }) => {
    ...
    return (
        <MotionDiv variants={itemVariants} initial='hidden' animate='visible' whileHover={{ scale: 1.02, transition: { duration: 0.2 }}} >
            <Card className='relative w-full  '>
                ...
                <Link href={`/summaries/${summary.id}`} className='block p-4 sm:p-6  ' >
                    <div className="flex flex-col gap-3 sm:gap-4 ">
                        ...
                        <div className="flex justify-between items-center mr-2 sm:mt-4 ">
